...
public class FragmentNotification extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        ((MainActivity) getActivity()).showToolbarTitle("NOTIFICATIONS");
...
//        notificationModels.add(new NotificationModel("Insurance Renewal","Congratulations! your insurance detail updated. Congratulations! your insurance detail updated."));
    ...
    private void getNotification() {
        ...
        if (CommonMethods.isNetwork(getActivity())) {
            ...
            Call<Common> call = apiInterface.get_user_all_notifications(Config.getSharedPreferences(getActivity(), "userID"), Config.getSharedPreferences(getActivity(), "accessToken"));
...
//                    if (c.getCode() == 100) {
                    if (c != null) {
                        if (c.getCode() == 100) {
                            if (c.getData().getUserAllNotificationList().size() > 0) {
                                txt_no_data.setVisibility(View.VISIBLE);
                                recyclerview_notification.setVisibility(View.GONE);
                                notificationModels.addAll(c.getData().getUserAllNotificationList());
                                adapterNotification = new AdapterNotification(getActivity(), notificationModels);
                                ...
                                txt_no_data.setVisibility(View.VISIBLE);
                                recyclerview_notification.setVisibility(View.GONE);
                            //    CommonMethods.showValidationPopup("No Notification found", getActivity());
                            }
